...
#' @return A list
#' \itemize{
#'     \item stats - 95% CI, mean, and std of bootstrap results for each feature weight for each PDM
  ...
  if (is.numeric(whPDM)){
    ...
    for (k in whPDM){
      ...
      if (k == 1){
        ...
        boot_pdm_results <- .BootPDM(x = x, y = y, M_tilde = M_tilde, W = NULL, Dt = Dt, Bsamp = Bsamp, WMi = WMi[1])

        # All other PDMs
      } else {

        boot_pdm_results <- .BootPDM(x = x, y = y, M_tilde = M_tilde, W = W[1:k-1], Dt = Dt, Bsamp = Bsamp, WMi = WMi[k])
      }
    ...
    # Bootstrap JointPDM
    boot_joint_results <- .BootPDMJoint(x = x, y = y, M_tilde = M_tilde, W = W, Dt = Dt, Bsamp = Bsamp, WMi = WMi)
