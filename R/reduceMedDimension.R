...
reduceMedDimension <- function(treatment, outcome, mediator, exVar = 0.9){
  ...
  M_reduced <- V[,1:nComps] %*% diag(diag_S)[1:nComps,1:nComps]
  tLoadingMatrix <- t(U[,1:nComps])
  ...
  # Return a list
  return(list('X'= X, "Y" = Y, "M" = mediator, "M_reduced" = M_reduced, "tLoadingMatrix" = tLoadingMatrix,
              'exVar' = exVar))
