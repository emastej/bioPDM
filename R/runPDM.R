#' .runPDM
#'
#' @description  Calculates PDM weights for each number of desired PDMs and organizes results
#'
...
#' @return A list
#' \itemize{
#'     \item W - matrix (b x q) Weights for each b feature calculated for each of the q PDMs
#'     \item Theta - matrix (5 x q) Mediation Path Coefficients for each of the q PDMs (gamma, gamma', alpha, beta, ab)
#'     \item Wfull - matrix (p x q) Weights mapped back onto original Mediator Matrix
#'     \item WMinit - matrix (b x q) Initial weights used for PDM calculation optimization
#'     \item WfullJoint - vector (p x 1) Weights for the JointPDM. Each Mediator feature has one weight (if applicable)
#'     }
#'
#' @noRd
#'
...
.runPDM <- function(x, y, M_tilde, Dt, nPDM, doJointPDM){
  ...
  # W is a matrix of weights from previous PDM calculated. Will be empty for first PDM
  W <- NULL
  # gamma, alpha, beta values
  Theta <- NULL
  # Full weights mapped back onto original Mediators (not just the mediator matrix with reduced dimension space)
  Wfull <- NULL
  ...
  # Return a list with final weights, theta, Weights mapped back onto the original mediators, initial weights, and JointPDM weights
  return(list('w_k' = W, 'w_k*Dt' = Wfull, 'JointW' = WJoint, 'Theta' = Theta, 'WMinit' = WMinit ))
