...
Radiomic-based principal direction of mediation (radPDM) is a downstream radiomics analysis method that uses high dimensional mediation to explore the causal pathway between an exposure and an outcome through a radiomic mediator. The goal of radPDM is to provide a method to perform mediation analysis on radiomic data where $p >> n$. The overall objective is to find groups of radiomic features that mediate the effect of an exposure, specifically a gene, on a phenotype of interest. 
...
\section{2. radPDM Overview}


\subsection{2.1 Workflow}

radPDM is a downstream radiomics analysis method adapted from Geuter et al 2020. It assumes the user has an exposure $X$, a phenotype $Y$, and radiomic data $M$ measured on the same subjects. radPDM assumes the radiomic data $M$ is high dimensional where $p >> n$.  Figure 2 displays a visual representation of the causal assumptions between $M$, $X$, and $Y$ data. The goal of radPDM is to find $q$ number of principal directions of mediation ($PDM$s) that each represent a weighted group of radiomic features that independently mediate the effect of $X$ on $Y$. radPDM finds the weights $w_{k,j}$ that maximize the indirect effect $|a_k b_k |$ between $X$ and $Y$ through $M$, under some constraints (Equation 1). 

<br> 

\[
PDM_{k,i} = \sum_{j=1}^{p} M_{ij} w_{k,j} \quad \textrm{for} \quad i=1,...n \quad \textrm{and} \quad k=1,...q
\]
\begin{equation}
  PDM_{k,i} = a_{0k} + a_k X_i + \epsilon_1 
\end{equation}
\[
Y_i = b_{0k} + c'_k X_i + b_k PDM_{k,i} + \epsilon_2 
\]
\[
...
* **Step 2:** Calculate the $PDMs$ 
  ...
  \[
  PDM_{1,i} = \sum_{j=1}^{b} \tilde{M_{ij}} w_{1,j}
  \]
  \[
  PDM_{1,i} = a_{01} + a_1 X_i
  \]
  \[
  Y_i = b_{01} + c'_1 X_i + b_1 PDM_{1,i}
  \]
  \[
  ...
  \[
  PDM_{k,i} = \sum_{j=1}^{b} \tilde{M_{ij}} w_{k,j}
  \]
  \[
  PDM_{k,i} = a_{0k} + a_k X_i
  \]
  \[
  Y_i = b_{0k} + c'_k X_i + b_k PDM_{k,i}
  \]
  \[
  \textrm{Find } w_k\quad\textrm{that maximizes }|a_k b_k|\quad s.t.\quad w^T_k w_k = 1\quad \textrm{and } w_k^T w_l = 0 \quad \textrm{where } l = 1,...k-1
  \]
    
  + **c.** Map weights back to the original radiomic feature space using $D$ found in Step I in order to understand which radiomic features are contributing the most to each $PDM$. 

  \[
  M = \tilde{M} D \rightarrow \tilde{M} = M D^{-1}
  \]
  \[
  \therefore \tilde{M}_{ij} = \sum_{z=1}^{p} M_{iz} D^{-1}_{zj}\quad\textrm{where } j = 1,...b
  \]
  \[
  \therefore PDM_{k,i} =  \sum_{j=1}^{b} (\sum_{z=1}^{p} M_{iz} D^{-1}_{zj})w_{k,j}
  \]
  
  + **(Optional) d.** Calculate the $JointPDM$. $JointPDM$ is a weighted combination of all calculated $PDM$. The indirect effect size is used as the weight for each $PDM$. This allows the user to understand the global role each radiomic feature has on the mediation between $X$ and $Y$ in addition to the role it plays in each individal $PDM$. 
  
  \[
  W_j = \sum_{k=1}^{q} (a_k b_k)w_{k,j} \quad\textrm{where } j = 1,...b
  \]
  \[
  JointPDM_i = \sum_{j=1}^{b} (\sum_{z=1}^{p} M_{iz} D^{-1}_{zj})W_j
  \]
...
The radPDM package has the following dependencies: graphics, NlcOptim, pracma, R.utils, and stats. The radPDM package includes 4 exported functions: 

* **reduceMedDimension():** Reduces the dimensions of the radiomic mediator space using singular value decomposition (SVD)
...
knitr::include_graphics("~/data_assumptions.png")
...
knitr::include_graphics("~/post_bioPDM.png")

```

```{r, echo=FALSE, warning = FALSE, out.width = '100%', fig.cap= "Workflow for radPDM to calculate principal directions of mediation (PDM). X is the exposure, Y is the phenotype of interest outcome, and M is high dimensional radiomic mediator.", fig.align='center'}
knitr::include_graphics("~/code_schematic.png")

```

\section{3. radPDM workflow with a synthetic dataset}

\subsection{3.1 Synthetic dataset}

To explore the basic utilization of radPDM, we consider a synthetic dataset with 1046 radiomic features ($M$), IDH mutation status ($X$), and survival outcome ($Y$) measured on 100 subjects. 
Radiomic features are centered and scaled. Survival outcome is reported as martingale residuals. 
...
The first step in radPDM is to reduce the dimension space of the mediator using singular value decomposition (SVD) such that $M$ = $\tilde{M} D$ has $b$ number of dimensions and $b < n << p$. The default parameter value is to pick $b$ number of dimensions that explain 90% of radiomic data variance, which ever is the smaller value. The output is a list that will be used in the next step. The output list contains $X$, $Y$, $\tilde{M}$, and $D$. 

```{r, cache = TRUE}
# library(radPDM) has already been installed 

## Reduce the dimensions of M
#reduced_mediator <- radPDM::reduceMedDimension(treatment = X, outcome = Y, mediator = M)
```


\subsection{3.3 Step II: Find PDMs}

Step II uses the output from Step I as input for getDirectionsOfMed() function to calculate the $PDM$s. This function allows the user to determine how many $PDM$s to calculate, if $JointPDM$ should be calculated, if a bootstrapping step should occur, and if the results should be saved to a specified directory. More details on the inputs and outputs of getDirectionsOfMed() can be found in the package function manual. 
...
#ex1_output <- radPDM::getDirectionsOfMed(data_list = reduced_mediator)
```

The ex1_output is a list that includes $w_{k,j}$, $D^{-1} w_{k,j}$ (weights after they have been mapped back to original mediator dimension space), $JointPDM$ $W_{j}$ weights, $Theta$ (the mediation path coefficients, $a, b, c, c'$, for each $PDM$), all $PDM$s, and $JointPDM$. Sections 3.4 and 3.5 show different ways to interact with and display output results. 

\subsubsection{Example 2: Bootstrapping Option}

getDirectionsofMed() also includes an option to bootstrap. The bootstrapping step bootstraps the data with replacement, calculates the $w_{k,j}$ weights for each bootstrapped sample, and calculates the 95% confidence interval (CI), mean, and standard deviation of the bootstrapped $w_{k,j}$ weights.  

For the sake of this example, the first 2 $PDM$s are calculated and 100 samples are bootstrapped. However, the default parameter value for bootstrapped samples is 1000. 

```{r, cache = TRUE}
# Run getDirectionsOfMed() with bootstrapping 
# Bootstrap the first 2 PDMs with 100 samples
# Do not calculate the JointPDM 
#ex2_output <- radPDM::getDirectionsOfMed(data_list = reduced_mediator, nPDM = 2,
#                doJointPDM = FALSE, doBootPDM = TRUE, BootSamp = 100)
```
...
#ex2_output$boot$w_kStats$pdm_1$feat_1
```

\subsubsection{Example 3: Saving Outputs}

Finally, there is an option to save your outputs. The user is required to set the folder for which the output files will be saved to. The saveResults = TRUE parameter value will automatically create a radPDM_results folder to save all results to. It then creates a new folder with the folder name corresponding to the date and time that getDirectionsOfMed() function was executed. The output list from getDirectionsOfMed() is saved as well as a text file that includes the number of $PDM$s calculated and the number of bootstrapped samples (if applicable). Additionally, there is an optional parameter to include free text notes into the file (Figure 5). 

```{r, cache = TRUE}
# Run getDirectionsOfMed() and save results 

# Specifiy the directory to save results to
#dir_to_save <- "~/"

# Optional Parameter of free text notes
#free_text <- "Using synthetic dataset, executed radPDM as an example for vignette"

#ex3_output <- radPDM::getDirectionsOfMed(data_list = reduced_mediator, nPDM = 3,
...
The final step of radPDM allows for the visualization of the mediation path coefficients corresponding to each calculated $PDM$ by using the output from Step II. There is a function to make a plot of the coefficients to understand how each mediation path changes with each passing $PDM$. There is also a function to print the path coefficients in a formatted table for easy viewing. In the following examples, the theta values from Example 1 are displayed. 

\subsubsection{Example: Plot Mediation Coefficients}


```{r, cache = TRUE}
# Plot Theta (mediation path coefficients)
#radPDM::plotMedPathCoeff(ex1_output$Theta) 
```
...
```{r, cache = TRUE}
# Print Theta (mediation path coefficients)
#radPDM::printMedPathCoeff(ex1_output$Theta) 
```
...
\subsection{3.5 Additional Ways to Interact with radPDM Results}
...
#ggplot(plot_df, aes(x=PDM1, y=PDM2, col = as.factor(X))) + geom_point(size = 3) +  
...
#PDM1_weights <- data.frame(features = names(M), w_k = abs(ex1_output$`w_k*Dt`[[1]]))
